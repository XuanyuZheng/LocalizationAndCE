function [cord_est, PHN_est, h_est_N, AOAs] = proposed_Uncons_with_Cons_ini_dxdy_DynGrid(y,SNRs,P,PHN_deg)
%proposed_estPHN_DynGrid 
%Estimate PHN, unconstrained formulation, but initialized with constrained formulation
% to get an initial sparse solution. Dynamic Grid Update
...
%     h_est = randn(P.N*P.Q,1)*(1/P.Q/P.N);      % % initialize to zeros
    ...
    thres = 1e-5;                   % stopping criteria for h after grid refinement
    err = Inf;                      % First error
    max_iter = 200;                  % max number of iteration
    mul = 2;
    bound = 2 * chi2inv(0.99, 2*P.N*P.Nr) + ...
        mul * 2*P.N*P.Nr * ((10.^(max(SNRs)/10)) * (1-cos(deg2rad(1*PHN_deg+1))));
%     bound = (  1 + mul * ((10.^(max(SNRs)/10)) * (1-cos(deg2rad(1*PHN_deg))))  );
...
%     obj_value = obj_lik(y, SNRs, PHN_est, dx_est, dy_est, h_est, P);
    while inner_iter <= max_iter && err >= thres
...
%                     cvx_solver mosek
        ...
        %             minimize( norm(x1,2)  )
                    minimize( sum(norms(x1))   )
                        subject to
                        sum_square_abs(y1-A*vec(transpose(x1))) <= (1.1*bound); %#ok<VUNUS>
                cvx_end
...
%             dx_est = dx_est + -0.1*dx_grad/norm(dx_grad);   % fixed step
%             
%             
%             % update sensing matrix Omega & update steerring matrix Ar
%             [~,Ar,AOA] = ...
...
%             dy_est = dy_est + -0.1*dy_grad/norm(dy_grad);   % fixed step
            ...
            for i = 1:P.N
               ...
               for j = 1:P.Nr
                   z = y((i-1)*P.Nr+j) - y_tilde_n(j);
                   y_ck = y_tilde_n(j);
                   PHN_est(j,i) = (imag(z*y_ck'))/(abs(y_ck)^2 + std_theta^-2);
%                    PHN_est(j,i) = wrapToPi(angle(y((i-1)*P.Nr+j)) - angle(y_tilde_n(j)));
%                    PHN_est(j,i) = 0;
